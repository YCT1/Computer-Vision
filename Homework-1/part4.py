...
from numpy.ma import diagonal, masked

"""
Comment about the two soluttion:

As we can see that, image rotates around the top left thus it goes to down and a little left position
thus we can see that affect in the result video

In contrast to centered rotation, everything is fine and image rotates around the center thus planes is same position like part3
To not to overshoot the frame, I have added padding so that image stays in the frame even after the both rotation
...
def rotate_image_by_center(image, angle):
    ...
    # Adding padding so that image not overshot the frame
    image = cv2.copyMakeBorder(image, 30, 30, 30, 30, cv2.BORDER_CONSTANT, None, value = 0)


    #find the center of the image  
    image_center = np.array(image.shape[1::-1])/2

    # Use get the rotation matrix
    rot_mat = cv2.getRotationMatrix2D(image_center, angle, 1.0)
    
   
    
    shape = np.array(image.shape[1::-1]).astype(np.int32)
    # wrap it 
    result = cv2.warpAffine(image, rot_mat,shape, flags=cv2.INTER_LINEAR)
    return result

def rotate_image_by_origin(image, angle,rotation_center):
    
    padding = 100

    rotation_center[0] += padding*2
    rotation_center[1] += padding
    # Adding padding so that image not overshot the frame
    image = cv2.copyMakeBorder(image, padding, padding, padding*2, 0, cv2.BORDER_CONSTANT, None, value = 0)
...
for i in range(0,video_length):
    ...
    for name_of_album_i in range(0,len(albumlist)):
        ...
        album = rotate_image_by_center(album,-60)

        # Used and deactivated for the other part of the homework
        # album = rotate_image_by_origin(album,-60,[0,0])
...
clip.write_videofile("part4_centered_rotation_video.mp4", codec="libx264")